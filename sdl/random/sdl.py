...
def create_sdl(p: int, m: int, n: int, o: int, steps_min: int, steps_max: int, filename=None,
    ...
    """Create o operation_pool, p partitions, m machines, and n jobs.
    Example: o = 10, p = 4, m = 6, n = 5
    operation_pool = [o1, o2, o3, o4, o5, o6, o7, o8, o9, o10]
    p = 4 means separate the above operation_pool set into 4 partitions
    ...
    m = 6 means we need 2 more machines that can do some operation_pool
    M5 = [o1, o5, o8, o9]
    M6 = [o4, o5, o7, o8]
    These additional machines will influence the schedule and they can possibly reduce the total makespan.
    Each job can have step_min to step_max operation_pool
    """
    ...
    machines, operations_to_machines = machine_factory.create_machine_partition(
        operations=operations, p=p, m=m, random_state=random_state)
    jobs = job_factory.random_jobs(operation_pool=operations, num_of_jobs=n, steps_min=steps_min,
